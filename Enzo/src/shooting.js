...
export function updateProjectiles(scene, delta, projectiles) {
    ...
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        if (!bullet.targetHit) {
            ...
            // First, check for raycast collision only if the bullet has traveled enough distance
            if (bullet.distanceTraveled >= 0.5) {
...
function handleProjectileHit(scene, bullet, hitObject, projectiles, index) {
    ...
    setTimeout(() => {
        ...
        projectiles.splice(index, 1);  // Ensure this bullet is removed from the projectiles array
